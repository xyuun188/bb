...
_REMOTE_MODEL_INFO_COMMAND = "\n".join(
    [
        ...
        "database_line=$(grep -m1 '^DATABASE_URL=' /etc/bb/bb-runtime.env 2>/dev/null || true)",
        'if [ -z "$key_line" ]; then',
        "  echo 'BB_SECURE_SETTINGS_KEY missing on platform server' >&2",
        "  exit 3",
        "fi",
        'if [ -z "$database_line" ]; then',
        "  echo 'DATABASE_URL missing on platform server' >&2",
        "  exit 4",
        "fi",
        "cd /data/bb/app",
        "sudo -u bb env BB_SECURE_SETTINGS_KEY=\"${key_line#BB_SECURE_SETTINGS_KEY=}\" DATABASE_URL=\"${database_line#DATABASE_URL=}\" PYTHONPATH=/data/bb/app ./.venv/bin/python - <<'PY'",
        "from __future__ import annotations",
        "",
        "import json",
        "import os",
