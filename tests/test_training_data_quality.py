    ...
    if isinstance(features, dict) and features and "training_label_contract" not in features:
        features["training_label_contract"] = compact_shadow_label_contract(
            build_shadow_label_contract(
                ...
                long_mfe_pct=max(float(sample.get("long_return_pct") or 0.0), 0.0),
                long_mae_pct=max(-float(sample.get("long_return_pct") or 0.0), 0.0),
                short_mfe_pct=max(float(sample.get("short_return_pct") or 0.0), 0.0),
                short_mae_pct=max(-float(sample.get("short_return_pct") or 0.0), 0.0),
