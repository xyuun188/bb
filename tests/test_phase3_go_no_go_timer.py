...
from scripts import install_phase3_go_no_go_timer as timer_script


def test_phase3_go_no_go_timer_service_is_read_only() -> None:
    service = timer_script.render_service()
    timer = timer_script.render_timer(on_calendar="*-*-* *:12,42:00")
    ...
    assert "Unit=bb-phase3-go-no-go.service" in timer


def test_phase3_go_no_go_timer_dry_run_does_not_connect(monkeypatch, capsys) -> None:
    ...
    assert "bb-phase3-go-no-go.service" in output
    assert "bb-phase3-go-no-go.timer" in output
    assert "run_phase3_go_no_go_report.py" in output
    assert "starts_trading_service" in output
    assert "False" in output


def test_phase3_go_no_go_run_now_only_starts_go_no_go_service(monkeypatch) -> None:
    commands: list[str] = []
    ...
    joined = "\n".join(commands)
    assert "systemctl start bb-phase3-go-no-go.service" in joined
