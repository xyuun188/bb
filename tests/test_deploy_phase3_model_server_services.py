...
def test_phase3_service_manifest_is_shadow_only_and_under_data_bb() -> None:
    ...
    assert by_slot["llm_decision_maker"]["served_model_name"] == "qwen3-32b-trade"
    assert by_slot["llm_decision_maker"]["model_dir"].endswith("Qwen--Qwen3-32B-AWQ")
    assert by_slot["llm_decision_maker"]["cuda_visible_devices"] == "0,1"
    assert by_slot["llm_decision_maker"]["tensor_parallel_size"] == 2
    assert by_slot["llm_expert_pool"]["served_model_name"] == "BB-FinQuant-Expert-14B"
    assert by_slot["llm_expert_pool"]["cuda_visible_devices"] == "2"
    for item in payload["services"]:
        assert item["model_dir"].startswith("/data/BB/models/")
...
def test_phase3_start_scripts_source_data_bb_env_and_bind_loopback() -> None:
    for spec in deploy.PHASE3_SERVICE_SPECS:
        source = spec.render_start_script()

        assert "source '/data/BB/env/phase3.env'" in source
        assert "--host 127.0.0.1" in source
        assert f"--port {spec.port}" in source
        assert f"--served-model-name '{spec.served_model_name}'" in source
        assert f"--tensor-parallel-size {spec.tensor_parallel_size}" in source
        assert "/data/BB/models/" in source
        assert "/data/trade_ai" not in source
        assert "/data/trade_models" not in source
...
def test_phase3_plan_only_does_not_connect(monkeypatch, capsys) -> None:
    def fail_connect(*_args, **_kwargs):
        raise AssertionError("plan-only must not connect to remote server")

    monkeypatch.setattr(deploy, "connect_remote_ssh", fail_connect)
