...
def test_market_analysis_distinguishes_observed_direction_from_open_permission() -> None:
    ...
    assert "dashboard.js?v=20260915-local-ml-stable-config-v2" in html
...
def test_dashboard_refreshes_active_local_ml_status() -> None:
    script = (PROJECT_ROOT / "web_dashboard/static/js/dashboard.js").read_text(encoding="utf-8")

    assert "if (isPageActive('ml-signal'))" in script
...
def test_expert_memory_table_uses_readable_structured_outcomes() -> None:
    ...
    assert 'class="expert-memory-table"' in html
    ...
    assert "计入资金费并扣除交易成本" in script
    assert "${escHtml(m.market_pattern || '-')}" not in script
    assert "${escHtml(m.lesson || '-')}" not in script
    assert '"realized_net_pnl_usdt"' in source
    assert '"net_return_after_all_cost_pct"' in source
    assert ".expert-memory-table" in style
    assert "min-width: 980px;" in style
    assert "@media (max-width: 1400px)" in style
    assert "overflow-wrap: anywhere;" in style
...
def test_position_history_separates_confirmed_close_from_official_settlement() -> None:
    ...
    assert "position.settlement_explanation" in script
    assert "positions_history_no_matching_row" in script
    assert "official_position_history_identity_unresolved" in script
...
def test_dashboard_static_bundle_version_tracks_local_ml_evidence_renderer() -> None:
    ...
    assert "/static/js/dashboard.js?v=20260915-local-ml-stable-config-v2" in html
