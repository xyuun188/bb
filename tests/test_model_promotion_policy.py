...
def _cost_complete_sample(
    net_return: float,
    *,
    authority: str = "rules",
    alignment: str = "supported_winning_side",
) -> dict[str, object]:
    return {
        "cost_complete": True,
        "fee_return_pct": 0.04,
            ...
            "tasks": {
                ...
                AUTHORITATIVE_REALIZED_RETURN_TASK: {
                    ...
                    "profit_training_contract": {
                        ...
                        "model_shadow_alignment": alignment,
                    },
                },
            },
        },
...
def _shadow_sample(net_return: float = 0.5, *, weight: float = 1.0) -> dict[str, object]:
    return {
        "decision_action": "long",
        "model_shadow_action": "long",
        "sample_weight": weight,
        "profit_supervision": {
            "version": PROFIT_SUPERVISION_VERSION,
            "fee_after_return_labels": {
                "complete": True,
                "long_net_return_after_all_cost_pct": net_return,
                "short_net_return_after_all_cost_pct": -net_return,
            },
            "tasks": {
                MARKET_OPPORTUNITY_TASK: {
                    "eligible": True,
...
def _return_report(values: tuple[float, ...]) -> dict[str, object]:
    return build_return_objective_report(
        shadow_samples=[_shadow_sample(value) for value in values],
        trade_samples=[_cost_complete_sample(0.1)],
    )
...
def test_return_objective_promotes_positive_fee_after_distribution() -> None:
    report = _return_report((0.8, 0.7, 0.6, 0.5, -0.1))

    assert report["promotion_ready"] is True
    assert report["optimization_target"] == PROFIT_TRAINING_TARGET
    assert report["separated_distributions"]["model_attributed_return"][
        PROFIT_TRAINING_TARGET
    ]["count"] == 5
    assert report["model_shadow_return_sample_count"] == 5
    assert report["model_live_return_sample_count"] == 0
    assert report["empirical_return_lower_hinge_pct"] > 0
    assert report["policy_provenance"]["sample_count"] == 5
...
def test_high_win_rate_negative_expectancy_cannot_promote() -> None:
    report = build_return_objective_report(
        shadow_samples=[*[_shadow_sample(0.1) for _ in range(9)], _shadow_sample(-2.0)],
        trade_samples=[_cost_complete_sample(0.1)],
    )
...
def test_low_win_rate_positive_payoff_still_requires_positive_lower_half() -> None:
    report = build_return_objective_report(
        shadow_samples=[_shadow_sample(4.0), _shadow_sample(-0.2), _shadow_sample(-0.2)],
        trade_samples=[_cost_complete_sample(0.1)],
    )
...
def test_legacy_profit_learning_labels_cannot_bypass_supervision_contract() -> None:
    ...
    assert "model_attributed_return_distribution_missing" in report[
        "blocking_reasons"
    ]
...
def test_phase3_promotion_uses_return_report_not_sample_count_or_win_rate() -> None:
    ...
    assert recommendation["live_ml_ready"] is True
...
def test_rules_trade_return_never_changes_model_promotion_distribution() -> None:
    shadow = [_shadow_sample(0.8), _shadow_sample(0.7), _shadow_sample(0.6), _shadow_sample(-0.1)]
    winning_rules = build_return_objective_report(
        shadow_samples=shadow,
        trade_samples=[
            _cost_complete_sample(50.0, alignment="supported_winning_side")
        ],
    )
    losing_rules = build_return_objective_report(
        shadow_samples=shadow,
        trade_samples=[
            _cost_complete_sample(-50.0, alignment="avoided_losing_side")
        ],
    )

    assert winning_rules["average_net_return_after_all_cost_pct"] == losing_rules[
        "average_net_return_after_all_cost_pct"
    ]
    assert winning_rules["profit_factor"] == losing_rules["profit_factor"]
    assert winning_rules["rules_canary_attribution"][
        "included_in_model_return_distribution"
    ] is False
    assert losing_rules["rules_canary_attribution"]["model_shadow_alignment_counts"] == {
        "avoided_losing_side": 1
    }


def test_model_live_trade_is_direct_model_return_evidence() -> None:
    report = build_return_objective_report(
        shadow_samples=[_shadow_sample(0.8), _shadow_sample(0.7), _shadow_sample(-0.1)],
        trade_samples=[_cost_complete_sample(0.6, authority="model", alignment="")],
    )

    assert report["model_shadow_return_sample_count"] == 3
    assert report["model_live_return_sample_count"] == 1
    assert report["sample_count"] == 4


def test_correlation_weight_changes_promotion_math() -> None:
    report = build_return_objective_report(
        shadow_samples=[
            _shadow_sample(5.0, weight=0.01),
            _shadow_sample(-0.2, weight=1.0),
        ],
        trade_samples=[_cost_complete_sample(0.1)],
    )
