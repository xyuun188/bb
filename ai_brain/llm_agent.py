...
def _format_entry_candidate_evidence(evidence: dict[str, Any]) -> str:
    ...
    return (
        ...
        f"{evidence.get('preferred_paper_observation_side') or 'neutral'}, "
        f"feature_score={_fmt_num(evidence.get('feature_opportunity_score'), 2)}. "
        f"{side_line('long')} | {side_line('short')}. "
        "preferred_side_by_evidence is the normal strategy direction. "
        "preferred_paper_observation_side is a paper-only diagnostic direction for collecting "
        "a tiny governed outcome when the fee-after mean is positive but its lower bound is not; "
        "it never grants execution, production permission, size, or leverage. "
