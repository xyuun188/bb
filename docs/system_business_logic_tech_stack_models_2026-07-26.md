...
生成时间：2026-07-26 19:08（Asia/Shanghai）

主代码基线：`ce737a6 Restore dashboard responsiveness`
...
本节来自 2026-07-26 18:59（Asia/Shanghai）执行的线上只读审计，不再引用旧审计文件。
...
| 当前 30 分钟市场决策 | 12 条，覆盖 9 个币种 |
| 当前持仓 | 2 个 |
| 持仓复盘决策 | 43 条 |
| 候选覆盖 | 当前窗口没有超期币种 |

两个服务在审计前不久重新进入 active 状态，因此当时连续性审计仍返回 `continuity_unproven`。这表示观察窗口还不够长，不表示交易主链已经停止。
...
| 事实和成本完整、可训练 | 57 |
| 事实不完整、继续隔离 | 449 |
| 可训练的亏损容忍 paper 训练结果 | 57 |
| 可训练的普通策略结果 | 0 |

当前已有 57 条完整可训练结果，而且这 57 条全部来自 `loss_tolerant_paper_training`。
...
当前代码不再要求“至少 3 个专家、至少 2 个支持通道”。那是旧口径，不能写进最新版业务逻辑。
...
2. 当前权威结果是 506 条，其中 57 条可训练、449 条继续隔离。
3. 57 条可训练结果全部来自受控亏损容忍 paper 训练，普通策略结果目前没有达到当前训练合同。
4. 模型服务器和 Quant API 已经在线，但 `live_routing_enabled=false`，模型没有生产交易权。
5. 当前 paper 训练不要求旧的“三专家/两支持通道”，而是要求明确方向、预测时间窗、完整合同和极小风险。
