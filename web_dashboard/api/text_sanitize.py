...
MOJIBAKE_MARKERS = (
    "锛", "绋", "鎵", "鍗", "骞", "浠", "褰", "撳", "墠", "鏃", "堕",
    "涓", "鍙", "瑙", "绯", "璁", "搴", "閫", "鎸", "鍦", "妯", "浜",
    "鐩", "椋", "楂", "鍚", "鍒", "寮", "淇", "惧", "獟", "€", "\ue1da",
    "閺", "閸", "瑜", "娴", "顫", "澧", "鑰",
)
...
EXACT_REPLACEMENTS = {
    "鍒嗘澶с€佹尝鍔ㄥ紓甯告垨娌℃湁鐩堝埄妯″瀷鍚屽悜纭锛屼繚鎸?1.15+ 楂橀棬妲涖€?":
        "分歧大、波动异常或没有盈利模型同向确认，保持 1.15+ 高门槛。",
    "ML 涓庢湇鍔″櫒鐩堝埄妯″瀷鍚屽悜涓旈鏈熸敹鐩婁负姝ｏ紝鍏佽 0.75+ 灏忎粨寮€浠撱€?":
        "ML 与服务器盈利模型同向且预期收益为正，允许 0.75+ 小仓开仓。",
    "ML 鎴栨湇鍔″櫒鐩堝埄妯″瀷涓?AI 鏂瑰悜鍚屽悜涓旈鏈熸敹鐩婁负姝ｏ紝鍏佽 0.85+ 灏忎粨寮€浠撱€?":
        "ML 或服务器盈利模型与 AI 方向同向且预期收益为正，允许 0.85+ 小仓开仓。",
    "涓撳鏂瑰悜涓€鑷翠笖棰勬湡鏀剁泭涓烘锛屽厑璁?0.90+ 寮€浠撱€?":
        "专家方向一致且预期收益为正，允许 0.90+ 开仓。",
    "浠婂ぉ杩樻病鏈夎甯佺鏂瑰悜鐨勭湡瀹炲钩浠撹褰曘€?":
        "今天还没有该币种方向的真实平仓记录。",
    "ML 褰撳墠杈炬爣锛屽弬涓庢満浼氳瘎鍒嗐€?":
        "ML 当前达标，参与机会评分。",
    "ML 褰撳墠澶勪簬瀛︿範瑙傚療涓垨璇ユ柟鍚戞湭杈炬爣锛屾湰娆℃満浼氳瘎鍒嗕笉浣跨敤 ML 鍔犲噺鍒嗐€?":
        "ML 当前处于学习观察中，或该方向未达标，本次机会评分不使用 ML 加减分。",
    "鏈轰細璇勫垎鎺掑悕闈犲墠锛屾湰杞繘鍏ユ墽琛岄槦鍒椼€?":
        "机会评分排名靠前，本轮进入执行队列。",
    "鏈轰細璇勫垎鎺掑悕闈犲墠锛屾湰杞凡杩涘叆鎵ц闃熷垪锛屾鍦ㄨ繘琛屼笅鍗曞墠妫€鏌ュ拰鎻愪氦璁㈠崟銆?":
    ...
    "璇ュ竵绉嶆渶杩戞粴鍔ㄧ湡瀹炰簭鎹熻繃澶?":
        "该币种最近滚动真实亏损过大",
    "璇ュ竵绉嶄粖澶╃疮璁＄湡瀹炰簭鎹熻秴杩囬檺鍒?":
        "该币种今天累计真实亏损超过限制",
    "璇ュ竵绉嶈繖涓柟鍚戠殑鐪熷疄浜忔崯宸茬粡瓒呰繃闄愬埗":
        "该币种这个方向的真实亏损已经超过限制",
    "璇ュ竵绉嶆鍦ㄨ鍙︿竴鏉″垎鏋愭祦绋嬪鐞嗭紝鏈寮€浠撴墽琛岃烦杩囷紝绛夊緟涓嬩竴杞噸鏂拌瘎浼般€?":
...
def _business_rewrite(text: str) -> str | None:
    ...
    if ("1-5" in text or "鐭" in text) and ("蹇" in text or "鎺цЕ" in text) and ("鍙嶅悜" in text or "娉㈠姩" in text):
...
def _mojibake_score(text: str) -> int:
    return sum(text.count(marker) for marker in MOJIBAKE_MARKERS) + text.count("�") * 3 + text.count("?")
