(() => {
  ...
  function renderStrategies() {
    ...
    $('#strategies').classList.remove('empty'); $('#strategies').innerHTML = rows.slice(0, 12).map((m) => line(first(m, ['name', 'strategy_name'], '当前策略'), `${first(m, ['status', 'market_regime', 'state'], '观察中')} · 净收益 ${first(m, ['net_return_after_all_cost_pct'], '--')} · ${first(m, ['reason', 'description'], '暂无说明')}`)).join('');
  }
