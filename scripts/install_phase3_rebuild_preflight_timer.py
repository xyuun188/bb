#!/usr/bin/env python3
"""Install the online systemd timer for the Phase 3 rebuild preflight report."""
...
REMOTE_OWNER = "bb:bb"
SERVICE_NAME = "bb-phase3-rebuild-preflight.service"
TIMER_NAME = "bb-phase3-rebuild-preflight.timer"
REMOTE_RUNTIME_ENV_PATH = "/etc/bb/bb-runtime.env"
REPORT_DIR_REL = "data/phase3_rebuild_preflight_reports"
DEFAULT_ON_CALENDAR = "*-*-* 00:40:00"
...
def render_service(*, remote_app_dir: str = REMOTE_APP_DIR, owner: str = REMOTE_OWNER) -> str:
    ...
    return f"""[Unit]
Description=BB Phase 3 rebuild preflight report
...
ExecStart=/bin/bash -lc 'cd {remote_app_dir} && if [ -x .venv/bin/python ]; then PY=.venv/bin/python; elif [ -x venv/bin/python ]; then PY=venv/bin/python; else PY=python3; fi; exec "$PY" scripts/run_phase3_rebuild_preflight.py --json-indent 0'
"""


def render_timer(*, on_calendar: str = DEFAULT_ON_CALENDAR) -> str:
    return f"""[Unit]
Description=Run BB Phase 3 rebuild preflight report
...
def install_timer(
    *,
    ...
    try:
        ...
        safe_print(run_remote_text(ssh, " && ".join(commands), timeout=180, check=True))
    finally:
        ssh.close()
