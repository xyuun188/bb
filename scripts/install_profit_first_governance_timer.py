#!/usr/bin/env python3
"""Install the online systemd timer for the Profit-First governance report."""
...
REMOTE_OWNER = "bb:bb"
SERVICE_NAME = "bb-profit-first-governance.service"
TIMER_NAME = "bb-profit-first-governance.timer"
REMOTE_RUNTIME_ENV_PATH = "/etc/bb/bb-runtime.env"
REPORT_DIR_REL = "data/profit_first_governance_reports"
DEFAULT_ON_CALENDAR = "*-*-* 00:24:00"
...
def render_service(*, remote_app_dir: str = REMOTE_APP_DIR, owner: str = REMOTE_OWNER) -> str:
    ...
    return f"""[Unit]
Description=BB Profit-First no-entry and losing-exit governance report
...
ExecStart=/bin/bash -lc 'cd {remote_app_dir} && if [ -x .venv/bin/python ]; then PY=.venv/bin/python; elif [ -x venv/bin/python ]; then PY=venv/bin/python; else PY=python3; fi; exec "$PY" scripts/run_profit_first_governance_report.py --json-indent 0'
"""


def render_timer(*, on_calendar: str = DEFAULT_ON_CALENDAR) -> str:
    return f"""[Unit]
Description=Run BB Profit-First governance report
