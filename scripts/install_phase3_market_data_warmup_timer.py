#!/usr/bin/env python3
"""Install the online Phase 3 market-data warmup timer."""
...
def render_service(
    *,
    ...
    return f"""[Unit]
Description=BB Phase 3 market-data warmup
...
def render_timer(*, on_calendar: str = DEFAULT_ON_CALENDAR) -> str:
    return f"""[Unit]
Description=Run BB Phase 3 market-data warmup
...
def install_timer(
    *,
    ...
    try:
        ...
        safe_print(run_remote_text(ssh, " && ".join(commands), timeout=420, check=True))
    finally:
        ssh.close()
