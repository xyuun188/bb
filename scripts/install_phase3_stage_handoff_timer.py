#!/usr/bin/env python3
"""Install the online systemd timer for the Phase 3 stage handoff report."""
...
REMOTE_OWNER = "bb:bb"
SERVICE_NAME = "bb-phase3-stage-handoff.service"
TIMER_NAME = "bb-phase3-stage-handoff.timer"
REMOTE_RUNTIME_ENV_PATH = "/etc/bb/bb-runtime.env"
REPORT_DIR_REL = "data/phase3_stage_handoff_reports"
REMOTE_STAGING_DIR_REL = "tmp/systemd-unit-stage"
DEFAULT_ON_CALENDAR = "*-*-* *:17,47:00"
...
def render_service(*, remote_app_dir: str = REMOTE_APP_DIR, owner: str = REMOTE_OWNER) -> str:
    ...
    return f"""[Unit]
Description=BB Phase 3 stage handoff report
...
ExecStart=/bin/bash -lc 'cd {remote_app_dir} && if [ -x .venv/bin/python ]; then PY=.venv/bin/python; elif [ -x venv/bin/python ]; then PY=venv/bin/python; else PY=python3; fi; exec "$PY" scripts/run_phase3_stage_handoff_report.py --json-indent 0'
"""


def render_timer(*, on_calendar: str = DEFAULT_ON_CALENDAR) -> str:
    return f"""[Unit]
Description=Run BB Phase 3 stage handoff report
