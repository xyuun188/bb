#!/usr/bin/env python3
"""Install the online systemd timer for Phase 3 OKX fact sync and report."""
...
REMOTE_OWNER = "bb:bb"
SERVICE_NAME = "bb-okx-daily-reconciliation.service"
TIMER_NAME = "bb-okx-daily-reconciliation.timer"
REMOTE_RUNTIME_ENV_PATH = "/etc/bb/bb-runtime.env"
REPORT_DIR_REL = "data/okx_daily_reconciliation_reports"
DEFAULT_ON_CALENDAR = "*-*-* 00:10:00"
...
def render_service(*, remote_app_dir: str = REMOTE_APP_DIR, owner: str = REMOTE_OWNER) -> str:
    ...
    return f"""[Unit]
Description=BB Phase 3 OKX fact sync and reconciliation report
...
def render_timer(*, on_calendar: str = DEFAULT_ON_CALENDAR) -> str:
    return f"""[Unit]
Description=Run BB OKX daily dry-run reconciliation report
